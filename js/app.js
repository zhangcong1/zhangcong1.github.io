...
// --- 自动替换被屏蔽的 sinaimg 图片为二次元（动漫）图片 ---
;(function(){
  // 你可以在下面添加你希望固定使用的动漫图片列表（直接可热链的图片 URL）
  var STATIC_ANIME_IMAGES = [
    // 可在此填入你喜欢的固定图片 URL，例如托管在 images/ 下的路径或可热链的第三方图床
    // 'https://your.cdn.example/anime1.jpg',
  ];

  // 是否使用 waifu.pics API 获取随机二次元图（推荐，质量和风格较好）
  var USE_WAIFU_API = true;
  var WAIFU_API = 'https://api.waifu.pics/sfw/waifu';

  var isSinaImg = function(url) {
    if(!url) return false;
    try { return /sinaimg\.cn/i.test(url); } catch(e) { return false; }
  }

  // 尝试获取一张动漫图；优先使用 STATIC_ANIME_IMAGES，否则调用 waifu.pics
  var getAnimeImage = function(timeoutMs) {
    timeoutMs = timeoutMs || 4000;
    return new Promise(function(resolve) {
      if(STATIC_ANIME_IMAGES && STATIC_ANIME_IMAGES.length) {
        resolve(STATIC_ANIME_IMAGES[getRndInteger(0, STATIC_ANIME_IMAGES.length - 1)]);
        return;
      }

      if(!USE_WAIFU_API) {
        // 回退到 picsum 的随机图（若没有静态动漫图且禁用 API）
        resolve('https://picsum.photos/1920/1080');
        return;
      }
      ...
      var to = setTimeout(function() {
        if(!settled) { settled = true; resolve('https://picsum.photos/1920/1080'); }
      }, timeoutMs);

      try {
        fetch(WAIFU_API).then(function(res){
          return res.json();
        }).then(function(json){
          if(settled) return;
          settled = true; clearTimeout(to);
          if(json && json.url) resolve(json.url);
          else resolve('https://picsum.photos/1920/1080');
        }).catch(function(){
          if(settled) return;
          settled = true; clearTimeout(to);
          resolve('https://picsum.photos/1920/1080');
        ...
        if(!settled) { settled = true; clearTimeout(to); resolve('https://picsum.photos/1920/1080'); }
      }
    });
  }

  var replaceBackgroundImages = function() {
    // data-background-image 元素
    $.each('[data-background-image]', function(el){
      try {
        var v = el.dataset.backgroundImage || el.getAttribute('data-background-image');
        if(v && isSinaImg(v)) {
          // 异步获取动漫图
          getAnimeImage().then(function(url){
            try { el.dataset.backgroundImage = url; el.setAttribute('data-background-image', url); } catch(e){}
          });
        }
      } catch(e){}
    });

    // img 元素（src 或 data-src）
    $.each('img', function(img){
      try {
        var src = img.getAttribute('src') || img.getAttribute('data-src') || img.dataset.src;
        if(src && isSinaImg(src)) {
          getAnimeImage().then(function(url){
            try {
              if(img.getAttribute('data-src') !== null || img.dataset.src) {
  ...
  // 错误回退：若图片加载失败，尝试用另一张动漫图替换
  document.addEventListener('error', function(e){
    var el = e.target;
    if(!el || el.tagName !== 'IMG') return;
    try {
      var cur = el.src || el.getAttribute('data-src') || '';
      // 若当前图片域名是 sinaimg 或已经标记过加载错误，则替换
      if(isSinaImg(cur) || el.dataset.loadErrorFlag === '1') {
        el.dataset.loadErrorFlag = '1';
        getAnimeImage().then(function(url){
          try {
            if(el.getAttribute('data-src') !== null || el.dataset.src) {
  ...
  // 初始化时运行一次替换
  try { replaceBackgroundImages(); } catch(e){}
